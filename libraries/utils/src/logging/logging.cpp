...
Logger::Logger(const std::string& class_name) : class_name_(class_name) {
  // Create or fetch logger for this class
  logger_ = spdlog::get(class_name_);
  if (!logger_) { logger_ = spdlog::stdout_color_mt(class_name_); }
